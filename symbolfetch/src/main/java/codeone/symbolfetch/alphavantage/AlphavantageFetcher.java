...
public class AlphavantageFetcher implements SymbolFetcher {
    ...
    public CompletableFuture<List<StockPrice>> fetch(String... symbols) {
        ...
        String url = String.format("https://www.alphavantage.co/query?function=BATCH_QUOTES_US&symbols=%s&apikey=GHKKM5CDKE525WQE", symbolString);
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create(url)).build();
        return httpClient.sendAsync(request, HttpResponse.BodyHandlers.ofString()).thenApply(HttpResponse::body).thenApply(this::readStockFromJson);
    }
